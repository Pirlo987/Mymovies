...
function Home() {
  ...
  const movies = discoverMovies.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview} poster={poster} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
